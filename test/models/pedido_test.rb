...
class PedidoTest < ActiveSupport::TestCase
  ...
  test 'pedido deve ter pizza' do
    ...
    pedido = Pedido.new(endereco_id: endereco.id, entregador_id: entregador.id, precoTotal: 1 * 23.0, status: 'Esperando Visualização', quantidadePizzas: '1', observacao: 'Nenhuma', nomeDestinatario: usuario.nome, contato: usuario.telefone, cpfDestinatario: usuario.cpf)
    assert_not pedido.save
  end
  ...
  test 'pedido deve ter endereco' do
    ...
    pedido = Pedido.new(entregador_id: entregador.id, precoTotal: 1 * 23.0, status: 'Esperando Visualização', quantidadePizzas: '1', observacao: 'Nenhuma', nomeDestinatario: usuario.nome, contato: usuario.telefone, cpfDestinatario: usuario.cpf, :pizza_attributes => { tamanho:'1', fatias:'6', preco:'23', observacao: 'nenhuma', sabor1_id:sabor.id, sabor2_id:nil})
    assert_not pedido.save
  end
end
